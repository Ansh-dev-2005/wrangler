...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  /**
   * A Directive can consist of a value. This visitor method extracts the value
   * and creates a token type <code>Value</code>.
   */
  @Override
  public RecipeSymbol.Builder visitValue(DirectivesParser.ValueContext ctx) {
    if (ctx.BYTE_SIZE() != null) {
      ...
      builder.addToken(new Text(str.substring(1, str.length() - 1)));
      ...
      throw new IllegalArgumentException("Unknown value type in context: " + ctx.getText());
    }
    return builder;
  }
